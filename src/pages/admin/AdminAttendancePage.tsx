...
import { useEffect, useState } from 'react';
import {
  UserCheck, Save, ChevronDown, Check, X,
  Users, ClipboardList, Info, CheckCircle2,
  XCircle, Clock, MapPin, Search,
} from 'lucide-react';
import { toast } from 'react-hot-toast';
import { labSessionService, registrationService, attendanceService } from '../../services';
import { Registration, LabSession, Attendance, MarkAttendanceRequest } from '../../types';

// ─── Palette (mirrors system-wide hash) ──────────────────────────────────────
...
const fmtDays = (days?: string[]) => {
  ...
  return [...days].sort((a, b) => ORDER.indexOf(a) - ORDER.indexOf(b)).map(d => SHORT[d] ?? d).join(' · ');
};

const buildSchedule = (reg: Registration): string => {
  const days  = fmtDays((reg as any).sessionDays);
  const start = fmtTime((reg as any).sessionStartTime);
  const end   = fmtTime((reg as any).sessionEndTime);
  return [days, start && end ? `${start} – ${end}` : start || end].filter(Boolean).join('  ');
};
...
const Avatar = ({ name }: { name: string }) => {
  ...
  return (
    <div style={{ width: 32, height: 32, borderRadius: 8, background: p.bg, border: `1.5px solid ${p.accent}28`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
      <span style={{ fontSize: 11, fontWeight: 700, color: p.accent, fontFamily: "'Cabinet Grotesk', sans-serif" }}>{init}</span>
    </div>
  );
};
...
type AttMap = Record<number, { present: boolean; notes: string }>;

// ─── Slot Block ───────────────────────────────────────────────────────────────

const SlotBlock = ({
  slot, attendanceMap, onToggle, onNotes,
  search, animDelay,
}: {
  slot:         SlotGroup;
  attendanceMap: AttMap;
  onToggle:     (regId: number) => void;
  onNotes:      (regId: number, val: string) => void;
  search:       string;
  animDelay:    string;
}) => {
  ...
  // Hide block if search yields nothing in this slot
  if (filtered.length === 0 && search) return null;

  const slotPresent = slot.regs.filter(r => attendanceMap[r.id]?.present).length;
  const slotTotal   = slot.regs.length;
  const slotPct     = slotTotal > 0 ? Math.round((slotPresent / slotTotal) * 100) : 0;
  const fillColor   = slotPct >= 80 ? '#22c55e' : slotPct >= 50 ? '#f59e0b' : '#ef4444';

  return (
    <div style={{ background: '#fff', border: '1px solid #e8e5df', borderRadius: 16, overflow: 'hidden', animation: `atp-rise .4s ease ${animDelay} both` }}>
      ...
      >
        {/* Number badge */}
        <div style={{ width: 46, height: 46, borderRadius: 12, flexShrink: 0, background: gp.bg, border: `1.5px solid ${gp.accent}28`, display: 'flex', flexDirection: 'column' as const, alignItems: 'center', justifyContent: 'center' }}>
          <span style={{ fontFamily: "'Cabinet Grotesk', sans-serif", fontSize: 8.5, fontWeight: 700, color: gp.accent, letterSpacing: '0.07em', textTransform: 'uppercase' as const, lineHeight: 1 }}>SLOT</span>
          <span style={{ fontFamily: "'Fraunces', serif", fontWeight: 700, fontSize: 20, color: gp.accent, lineHeight: 1 }}>{slot.slotNumber}</span>
        </div>

        {/* Info */}
        <div style={{ flex: 1, minWidth: 0 }}>
          <p style={{ fontFamily: "'Fraunces', serif", fontWeight: 600, fontSize: 15.5, color: '#18181b', margin: '0 0 5px' }}>
            {slot.timeSlotDisplayName}
          </p>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' as const }}>
            {slot.schedule && (
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 12, color: '#64748b', fontFamily: "'Cabinet Grotesk', sans-serif", fontWeight: 500 }}>
                <Clock size={11} color="#94a3b8" strokeWidth={2} />{slot.schedule}
              </span>
            )}
            {slot.labRoom && (
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 12, color: '#64748b', fontFamily: "'Cabinet Grotesk', sans-serif", fontWeight: 500 }}>
                <MapPin size={11} color="#94a3b8" strokeWidth={2} />{slot.labRoom}
              </span>
            )}
          </div>
        </div>

        {/* Attendance rate mini-widget */}
        <div style={{ flexShrink: 0, display: 'flex', flexDirection: 'column' as const, alignItems: 'flex-end', gap: 5, minWidth: 120 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ fontFamily: "'Fraunces', serif", fontWeight: 700, fontSize: 15, color: fillColor }}>{slotPct}%</span>
            <span style={{ fontSize: 11.5, color: '#94a3b8', fontFamily: "'Cabinet Grotesk', sans-serif", fontWeight: 500 }}>
              {slotPresent}/{slotTotal} present
            </span>
          </div>
          <div style={{ width: 110, height: 4, background: '#e8e5df', borderRadius: 999, overflow: 'hidden' }}>
            <div style={{ height: '100%', width: `${slotPct}%`, background: fillColor, borderRadius: 999, transition: 'width .5s ease' }} />
          </div>
        </div>
      ...
      {!collapsed && (
        <>
          {/* Column headers */}
          <div style={{ display: 'grid', gridTemplateColumns: '56px 1fr 120px 1fr', padding: '8px 20px', background: '#fafaf9', borderBottom: '1px solid #f1f0ec' }}>
            {['Status', 'Student', 'ID Number', 'Notes'].map((h, i) => (
          ...
          {filtered.map((reg, i) => {
            ...
            return (
              <div
                key={reg.id}
                style={{
                  display: 'grid', gridTemplateColumns: '56px 1fr 120px 1fr',
                  alignItems: 'center', padding: '10px 20px',
                  borderBottom: i < filtered.length - 1 ? '1px solid #f5f4f0' : 'none',
                  background: att.present ? '#f8fef9' : 'transparent',
                  transition: 'background .12s',
                  animation: `atp-fadein .2s ease ${i * 0.025}s both`,
                }}
                onMouseEnter={e => { if (!att.present) e.currentTarget.style.background = '#fafaf9'; }}
                onMouseLeave={e => { e.currentTarget.style.background = att.present ? '#f8fef9' : 'transparent'; }}
              >
                {/* Toggle */}
                <div>
                  <button
                    ...
                    style={{
                      ...
                      background: att.present ? '#22c55e' : '#f1f5f9',
                      color: att.present ? '#fff' : '#94a3b8',
                      boxShadow: att.present ? '0 2px 8px #22c55e38' : 'none',
                    }}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0, paddingRight: 12 }}>
                  <Avatar name={reg.studentName || 'Student'} />
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontFamily: "'Fraunces', serif", fontWeight: 600, fontSize: 14, color: '#18181b', whiteSpace: 'nowrap' as const, overflow: 'hidden', textOverflow: 'ellipsis' }}>
                      {reg.studentName}
                    </div>
                    <div style={{ fontSize: 11, color: '#94a3b8', fontFamily: "'Cabinet Grotesk', sans-serif", fontWeight: 500, marginTop: 1 }}>
                      {reg.programName ?? ''}
                    </div>
                  </div>
                </div>
                ...
                <input
                  type="text"
                  value={att.notes}
                  onChange={e => onNotes(reg.id, e.target.value)}
                  placeholder="Add a note…"
                  style={{
                    width: '100%', padding: '7px 10px', fontSize: 12.5,
                    border: '1.5px solid transparent', borderRadius: 7,
                    background: 'transparent', color: '#334155', outline: 'none',
                    fontFamily: "'Cabinet Grotesk', sans-serif", transition: 'all .12s',
                    boxSizing: 'border-box' as const,
                  }}
                  onFocus={e => { e.target.style.borderColor = '#6366f1'; e.target.style.background = '#fff'; e.target.style.boxShadow = '0 0 0 3px #eef2ff'; }}
                  onBlur={e => { e.target.style.borderColor = 'transparent'; e.target.style.background = 'transparent'; e.target.style.boxShadow = 'none'; }}
                />
              </div>
            );
          })}
          ...
          {filtered.length === 0 && (
            <div style={{ padding: '24px 20px', textAlign: 'center', color: '#94a3b8', fontSize: 13, fontFamily: "'Cabinet Grotesk', sans-serif" }}>
              No students match your search in this slot
            </div>
          )}
        </>
      )}
...
const AdminAttendancePage = () => {
  ...
  const [saving, setSaving]                         = useState(false);
  const [search, setSearch]                         = useState('');
  const [attendanceMap, setAttendanceMap]           = useState<AttMap>({});
  ...
  useEffect(() => {
    labSessionService.getAll()
      .then(d => setSessions(d.filter((s: LabSession) => s.status === 'OPEN')))
      .catch(() => toast.error('Failed to load sessions'))
      .finally(() => setLoading(false));
  }, []);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      const confirmed = regs.filter((r: Registration) => r.status === 'CONFIRMED');
      setRegistrations(confirmed);
      setExistingAttendance(att);
      // Seed attendance map from existing records; default absent
      const m: AttMap = {};
      confirmed.forEach((reg: Registration) => {
        const ex = att.find((a: Attendance) => a.registrationId === reg.id);
        m[reg.id] = { present: ex?.present ?? false, notes: ex?.notes ?? '' };
      });
      setAttendanceMap(m);
    }).catch(() => toast.error('Failed to load data'))
      .finally(() => setLoadingList(false));
  ...
  const toggle  = (id: number) => setAttendanceMap(p => ({ ...p, [id]: { ...p[id], present: !p[id]?.present } }));
  const setNote = (id: number, notes: string) => setAttendanceMap(p => ({ ...p, [id]: { ...p[id], notes } }));
  const markAll = (present: boolean) => {
    const m: AttMap = {};
    registrations.forEach(r => { m[r.id] = { present, notes: attendanceMap[r.id]?.notes ?? '' }; });
    setAttendanceMap(m);
  ...
  // ── Build slot groups from registrations (slotNumber from RegistrationDTO) ──
  const groupMap = new Map<number, SlotGroup>();
  for (const reg of registrations) {
    const key = (reg as any).slotNumber ?? 0;
    if (!groupMap.has(key)) {
      groupMap.set(key, {
        slotNumber:          (reg as any).slotNumber   ?? 0,
        timeSlotDisplayName: (reg as any).timeSlotDisplayName ?? `Slot ${(reg as any).slotNumber ?? 0}`,
        labRoom:             (reg as any).labRoom,
  ...
  const slotGroups = Array.from(groupMap.values()).sort((a, b) => a.slotNumber - b.slotNumber);

  const selectedSessionObj = sessions.find(s => s.id === selectedSession);
  const totalPresent       = Object.values(attendanceMap).filter(a => a.present).length;
  const totalStudents      = registrations.length;
  const totalPct           = totalStudents > 0 ? Math.round((totalPresent / totalStudents) * 100) : 0;
  const pctColor           = totalPct >= 70 ? '#22c55e' : totalPct >= 50 ? '#f59e0b' : '#ef4444';

  const SpinnerInline = () => (
    <div style={{ width: 13, height: 13, border: '2px solid rgba(255,255,255,.3)', borderTopColor: '#fff', borderRadius: '50%', animation: 'atp-spin .7s linear infinite', flexShrink: 0 }} />
  );

  if (loading) return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '60vh', gap: 12 }}>
      <div style={{ width: 28, height: 28, border: '2.5px solid #e2e8f0', borderTopColor: '#6366f1', borderRadius: '50%', animation: 'atp-spin .75s linear infinite' }} />
      <span style={{ fontSize: 13, color: '#94a3b8', fontFamily: "'Cabinet Grotesk', sans-serif" }}>Loading sessions…</span>
      <style>{`@keyframes atp-spin { to { transform: rotate(360deg); } }`}</style>
    </div>
  );
  ...
  return (
    <>
      <style>{`
        ...
        @keyframes atp-rise   { from { opacity:0; transform:translateY(14px); } to { opacity:1; transform:translateY(0); } }
        @keyframes atp-fadein { from { opacity:0; } to { opacity:1; } }

        .atp-page { font-family:'Cabinet Grotesk',sans-serif; background:#f5f4f0; min-height:100vh; padding:44px 40px; }
        .atp-wrap { max-width:1100px; margin:0 auto; animation:atp-rise .35s ease both; }
        ...
        .atp-btn:active { transform:scale(.97); }
        .atp-btn-present { color:#15803d; border-color:#bbf7d0; background:#f0fdf4; }
        .atp-btn-present:hover { background:#dcfce7; }
        .atp-btn-absent  { color:#dc2626; border-color:#fecaca; background:#fef2f2; }
        .atp-btn-absent:hover  { background:#fee2e2; }
        .atp-btn-save {
          color:#fff; border-color:#18181b; background:#18181b;
          box-shadow:0 1px 6px rgba(0,0,0,.15);
        }
        .atp-btn-save:hover:not(:disabled) { background:#27272a; box-shadow:0 4px 16px rgba(0,0,0,.2); transform:translateY(-1px); }
        .atp-btn-save:disabled { opacity:.45; cursor:not-allowed; }

        .atp-search { position:relative; flex:1; max-width:280px; }
        .atp-search input {
          width:100%; padding:8px 12px 8px 34px;
        ...
        .atp-search input:focus { border-color:#6366f1; background:#fff; box-shadow:0 0 0 3px #eef2ff; }
        .atp-search svg { position:absolute; left:10px; top:50%; transform:translateY(-50%); pointer-events:none; }

        .atp-empty {
          display:flex; flex-direction:column; align-items:center;
          justify-content:center; padding:72px 40px; gap:10px; text-align:center;
        ...
        @media (max-width:700px) { .atp-page { padding:24px 16px; } }
      `}</style>
      ...
      <div className="atp-page">
        <div className="atp-wrap">
          ...
          <div style={{ marginBottom: 26 }}>
            ...
            <p style={{ fontSize: 13.5, color: '#94a3b8', marginTop: 5, fontWeight: 500 }}>
              Mark student attendance grouped by time slot
            </p>
          </div>

          {/* ── Config card ────────────────────────────────────────────────── */}
          <div className="atp-config" style={{ marginBottom: 16 }}>
            {/* Session select */}
            <div style={{ flex: 2, minWidth: 220 }}>
              <label style={{ display: 'block', fontSize: 11.5, fontWeight: 600, color: '#64748b', marginBottom: 5, fontFamily: "'Cabinet Grotesk', sans-serif", letterSpacing: '0.03em' }}>Lab Session</label>
              <div style={{ position: 'relative' }}>
                <select
                  value={selectedSession ?? ''}
                  onChange={e => { setSearch(''); setRegistrations([]); setSelectedSession(e.target.value ? parseInt(e.target.value) : null); }}
                  style={{ ...inp, appearance: 'none', paddingRight: 34, cursor: 'pointer' }}
                  onFocus={e => { e.target.style.borderColor = '#6366f1'; e.target.style.background = '#fff'; e.target.style.boxShadow = '0 0 0 3px #eef2ff'; }}
                  onBlur={e => { e.target.style.borderColor = '#e2e8f0'; e.target.style.background = '#f8fafc'; e.target.style.boxShadow = 'none'; }}
                >
            ...
            {/* Date picker */}
            <div style={{ width: 170 }}>
              <label style={{ display: 'block', fontSize: 11.5, fontWeight: 600, color: '#64748b', marginBottom: 5, fontFamily: "'Cabinet Grotesk', sans-serif", letterSpacing: '0.03em' }}>Date</label>
              <input
                type="date" value={selectedDate}
                onChange={e => setSelectedDate(e.target.value)}
                style={inp}
            ...
            {/* Session name pill */}
            {selectedSessionObj && (
              <div style={{ padding: '9px 14px', background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 9, alignSelf: 'flex-end' }}>
                <p style={{ fontSize: 10, fontWeight: 700, color: '#94a3b8', margin: 0, letterSpacing: '0.07em', textTransform: 'uppercase', fontFamily: "'Cabinet Grotesk', sans-serif" }}>Session</p>
                <p style={{ fontSize: 13.5, fontWeight: 700, color: '#18181b', margin: '2px 0 0', fontFamily: "'Cabinet Grotesk', sans-serif" }}>{selectedSessionObj.name}</p>
              </div>
            )}
          </div>
          ...
          {!selectedSession ? (
            <div className="atp-empty">
              ...
              <p style={{ fontFamily: "'Fraunces', serif", fontSize: 20, fontWeight: 600, color: '#18181b', margin: 0 }}>Select a session</p>
              <p style={{ fontSize: 13.5, color: '#94a3b8', margin: 0 }}>Choose a lab session and date above to begin marking attendance</p>
            </div>

          ) : loadingList ? (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, padding: '72px 0' }}>
              <div style={{ width: 26, height: 26, border: '2.5px solid #e2e8f0', borderTopColor: '#6366f1', borderRadius: '50%', animation: 'atp-spin .75s linear infinite' }} />
              <span style={{ fontSize: 13, color: '#94a3b8', fontFamily: "'Cabinet Grotesk', sans-serif" }}>Loading registrations…</span>
            </div>
            ...
            <div className="atp-empty">
              ...
              <p style={{ fontFamily: "'Fraunces', serif", fontSize: 20, fontWeight: 600, color: '#18181b', margin: 0 }}>No confirmed students</p>
              <p style={{ fontSize: 13.5, color: '#94a3b8', margin: 0 }}>There are no confirmed registrations for this session</p>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div className="atp-summary">
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 20, flexWrap: 'wrap' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
                    ...
                    <div>
                      <p style={{ fontFamily: "'Fraunces', serif", fontWeight: 600, fontSize: 21, color: '#18181b', margin: 0, lineHeight: 1 }}>{totalPresent}</p>
                      <p style={{ fontSize: 11, color: '#94a3b8', margin: 0, fontWeight: 600 }}>Present</p>
                    </div>
                  </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
                    ...
                    <div>
                      <p style={{ fontFamily: "'Fraunces', serif", fontWeight: 600, fontSize: 21, color: '#18181b', margin: 0, lineHeight: 1 }}>{totalStudents - totalPresent}</p>
                      <p style={{ fontSize: 11, color: '#94a3b8', margin: 0, fontWeight: 600 }}>Absent</p>
                    </div>
                  </div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10 }}>
                      <span style={{ fontSize: 11, color: '#94a3b8', fontWeight: 600 }}>Overall rate</span>
                      <span style={{ fontFamily: "'Fraunces', serif", fontWeight: 700, fontSize: 13, color: pctColor }}>{totalPct}%</span>
                    </div>
                    <div style={{ width: 120, height: 5, background: '#f1f5f9', borderRadius: 999, overflow: 'hidden' }}>
                      <div style={{ height: '100%', width: `${totalPct}%`, background: pctColor, borderRadius: 999, transition: 'width .5s ease' }} />
                    </div>
                    <span style={{ fontSize: 10.5, color: '#94a3b8' }}>{slotGroups.length} slot{slotGroups.length !== 1 ? 's' : ''} · {totalStudents} students</span>
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
                  ...
                  <button className="atp-btn atp-btn-present" onClick={() => markAll(true)}>
                    <Check size={13} strokeWidth={2.5} /> All Present
                  </button>
                  <button className="atp-btn atp-btn-absent" onClick={() => markAll(false)}>
                    <X size={13} strokeWidth={2.5} /> All Absent
                  </button>
                  <button className="atp-btn atp-btn-save" onClick={handleSave} disabled={saving}>
                    {saving ? <><SpinnerInline /> Saving…</> : <><Save size={13} /> Save Attendance</>}
                  </button>
                </div>
              </div>
              ...
              {/* Bottom save bar (when list is long) */}
              {totalStudents > 8 && (
                <div style={{ background: '#fff', border: '1px solid #e8e5df', borderRadius: 12, padding: '14px 20px', display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
                  <button className="atp-btn atp-btn-save" onClick={handleSave} disabled={saving}>
                    {saving ? <><SpinnerInline /> Saving…</> : <><Save size={13} /> Save Attendance</>}
                  </button>
                </div>
